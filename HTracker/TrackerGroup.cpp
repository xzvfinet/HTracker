...
void TrackerGroup::match( Mat img, vector<Rect> rects )
{
	...
	for(int i=0; i<trackers.size(); i++) {
		vector<int> rectIndex; // store the index of rects that are close to trackers[i]
		for(int j=0; j<rects.size(); j++) {
			if( !isRectUsed[j] && rectDistance(trackers[i].getRect(), rects[j]) < this->rectDistThreshold ) {	// when rects[j] is unused and 
				this->countNoNeighbor=0;
				//cout<<trackers[i].getRect()<<" is close to "<<rects[j]<<endl;
				rectIndex.push_back(j);
			} else {
				// too far to match
				//cout<<trackers[i].getRect()<<" tracker is not close to "<<rects[j]<<endl;
		...
		if(rectIndex.size() > 1) {
			for (int j=0; j<rectIndex.size(); j++) {
				if( !trackers[i].isMatch && !isRectUsed[ rectIndex[j] ] ) {	// when trackers[i] is not matched with other and rect is not used
					if ( trackers[i].match(img, rects[ rectIndex[j] ]) ) {
						/***********************************************/
						/* MATCHED                                     */
						/***********************************************/
						//cout<<i<<" tracker is matched with "<<j<<endl;
